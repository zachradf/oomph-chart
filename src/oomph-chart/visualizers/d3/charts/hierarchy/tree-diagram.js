...
export default function createTreeDiagram(data, options, chartComponents) {
  const {
    ...
    strokeColor,
    margin,
    strokeWidth = 1.5, // Default value if not provided in options
    opacity,
    childTextSize,
    fontColor,
  } = options;
  ...
  const topPadding = margin.top + radius; // Adjust the padding here based on your requirements
  const sidePadding = margin.left + radius; // Adjust the padding here based on your requirements
